...
describe('Game', () => {
  ...
  it('playTurn throws an error when trying to execute a love action without passing a target opponent', () => {
    ...
    expect(() => game.playTurn(game.player1, ActionType.LOVE_SPECIAL_ACTION)).toThrowError(/target player/i);

    expect(() => game.playTurn(game.player1, ActionType.LOVE_NORMAL_ACTION)).toThrowError(/target player/i);
  });
  ...
  it('playTurn sets gives the correct special love action points to the opponent', () => {
    ...
    expect(game.player2.loveTaken).toBe(game.player1.character.specialLovePower);
  });
});
